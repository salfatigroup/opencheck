...
export class AgentFactory {
  ...
  static buildSystemPrompt(testCase: string, baseUrl: string): string {
    return [
      ...
      "",
      "Token efficiency — minimize snapshot usage:",
      "- IMPORTANT: browser_evaluate returns only the JavaScript result, not the full page tree,",
      "  making it far more token-efficient than browser_snapshot or browser_wait_for.",
      "- Use browser_evaluate with JavaScript to check for specific elements, text, or loading states",
      "  (e.g., document.querySelector('.loading'), document.body.innerText.includes('Dashboard')).",
      "- When polling for status changes or waiting for loading to complete, always use browser_evaluate",
      "  with a targeted JS expression that extracts only the relevant data from the page.",
      "- Only use browser_snapshot when you need to discover elements you don't already know about.",
      "- If browser_evaluate returns empty or unexpected results, take ONE browser_snapshot to inspect",
      "  the page structure, then resume using browser_evaluate for subsequent checks.",
      "",
      "Instructions:",
